...
public class OverMessageHistory extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    private final MessageObject messageObject;
    private List<EditedMessage> messages;
    private int rowCount;
    private RecyclerListView listView;

    public OverMessageHistory(MessageObject messageObject) {
    ...
    public View createView(Context context) {
        ...
        ListAdapter adapter;
        listView.setAdapter(adapter = new OverMessageHistory.ListAdapter(context));
    ...
    public boolean onFragmentCreate() {
        ...
        NotificationCenter.getInstance(UserConfig.selectedAccount).addObserver(this, OverConstants.MESSAGE_EDITED_NOTIFICATION);
    ...
    public void didReceivedNotification(int id, int account, Object... args) {
        if (id == OverConstants.MESSAGE_EDITED_NOTIFICATION) {
    ...
    public void onFragmentDestroy() {
        ...
        NotificationCenter.getInstance(UserConfig.selectedAccount).removeObserver(this, OverConstants.MESSAGE_EDITED_NOTIFICATION);
    }
    ...
    private class ListAdapter extends RecyclerListView.SelectionAdapter {
        ...
        public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            View view;
            if (viewType == 1) {
                view = new OverMessageCell(context, getParentActivity(), OverMessageHistory.this);
            } else {
                view = null;
            }
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            if (holder.getItemViewType() == 1) {
                var ayuMessageDetailCell = (OverMessageCell) holder.itemView;
        ...
        private MessageObject createMessageObject(EditedMessage editedMessage) {
            ...
            OverMessageUtils.map(editedMessage, msg, currentAccount);
            OverMessageUtils.mapMedia(editedMessage, msg);

            msg.date = editedMessage.entityCreateDate;
            msg.edit_hide = true;

            // let file be downloaded
            if (editedMessage.documentType == OverConstants.DOCUMENT_TYPE_FILE && Objects.equals(editedMessage.mediaPath, FileLoader.getInstance(currentAccount).getPathToMessage(messageObject.messageOwner).getAbsolutePath())) {
