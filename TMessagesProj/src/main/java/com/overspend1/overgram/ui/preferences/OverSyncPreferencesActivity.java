...
import com.overspend1.overgram.sync.OverSyncConfig;
...
public class OverSyncPreferencesActivity extends BasePreferencesActivity implements NotificationCenter.NotificationCenterDelegate {
    ...
    public boolean onFragmentCreate() {
        ...
        NotificationCenter.getGlobalInstance().addObserver(this, OverConstants.AYUSYNC_LAST_SENT_CHANGED);
        NotificationCenter.getGlobalInstance().addObserver(this, OverConstants.AYUSYNC_LAST_RECEIVED_CHANGED);
        NotificationCenter.getGlobalInstance().addObserver(this, OverConstants.AYUSYNC_REGISTER_STATUS_CODE_CHANGED);
        return true;
    }

    @Override
    public void didReceivedNotification(int id, int account, Object... args) {
        if (id == OverConstants.AYUSYNC_STATE_CHANGED) {
            if (listAdapter != null) {
                listAdapter.notifyItemChanged(ayuSyncStatusRow);
            }
        } else if (id == OverConstants.AYUSYNC_LAST_SENT_CHANGED) {
            if (listAdapter != null) {
                listAdapter.notifyItemChanged(lastSentEventRow);
            }
        } else if (id == OverConstants.AYUSYNC_LAST_RECEIVED_CHANGED) {
            if (listAdapter != null) {
                listAdapter.notifyItemChanged(lastReceivedEventRow);
            }
        } else if (id == OverConstants.AYUSYNC_REGISTER_STATUS_CODE_CHANGED) {
            if (listAdapter != null) {
    ...
    public void onFragmentDestroy() {
        ...
        NotificationCenter.getGlobalInstance().removeObserver(this, OverConstants.AYUSYNC_LAST_SENT_CHANGED);
        NotificationCenter.getGlobalInstance().removeObserver(this, OverConstants.AYUSYNC_LAST_RECEIVED_CHANGED);
        NotificationCenter.getGlobalInstance().removeObserver(this, OverConstants.AYUSYNC_REGISTER_STATUS_CODE_CHANGED);
    }
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        if (position == serverURLRow) {
            OverUi.spawnEditBox(
                    ...
                    OverConfig::getSyncServerURL,
                    "syncServerURL",
                    OverConstants.DEFAULT_AYUSYNC_SERVER,
                    s -> {
            ...
            OverConfig.editor.putBoolean("syncEnabled", OverConfig.syncEnabled ^= true).apply();
            ((TextCheckCell) view).setChecked(OverConfig.syncEnabled);

            OverSyncController.nullifyInstance();
            OverSyncController.create();
        } else if (position == visitProfileRow) {
            Browser.openUrl(getParentActivity(), OverSyncConfig.getProfileURL());
        } else if (position == forceSyncRow) {
            OverSyncController.getInstance().forceSync();
        } else if (position == deviceIdentifierRow) {
            AndroidUtilities.addToClipboard(OverUtils.getDeviceIdentifier());
            ...
            OverConfig.editor.putBoolean("useSecureConnection", OverConfig.useSecureConnection ^= true).apply();
            ((TextCheckCell) view).setChecked(OverConfig.useSecureConnection);
        ...
        public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position, boolean payload) {
            switch (holder.getItemViewType()) {
                ...
                case 2:
                    ...
                    if (position == serverURLRow) {
                        textCell.setTextAndValue(LocaleController.getString(R.string.AyuSyncServerURL), OverConfig.getSyncServerURL(), true);
                    } else if (position == serverTokenRow) {
                        textCell.setTextAndValue(LocaleController.getString(R.string.AyuSyncServerToken), OverConfig.getSyncServerToken(), true);
                    } else if (position == visitProfileRow) {
                        ...
                        textCell.setTextAndValue(LocaleController.getString(R.string.AyuSyncDeviceIdentifier), OverUtils.getDeviceIdentifier(), true);
                    } else if (position == lastSentEventRow) {
                        var last = OverSyncState.getLastSent() != 0
                                ? LocaleController.formatDateAudio(OverSyncState.getLastSent(), true)
                        ...
                        var last = OverSyncState.getLastReceived() != 0
                                ? LocaleController.formatDateAudio(OverSyncState.getLastReceived(), true)
                                : LocaleController.getString(R.string.AyuSyncLastEventNever);
                        ...
                        var val = OverSyncState.getRegisterStatusCode();
                        if (val == 0) {
                ...
                case 5:
                    ...
                    if (position == syncEnabledRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString(R.string.AyuSyncEnable), OverConfig.syncEnabled, false);
                    } else if (position == useSecureConnectionRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString(R.string.AyuSyncUseSecureConnection), OverConfig.useSecureConnection, false);
                    }
                    break;
            }
        }
